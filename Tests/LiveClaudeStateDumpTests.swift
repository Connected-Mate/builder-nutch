...
final class LiveClaudeStateDumpTests: XCTestCase {
    func testDumpLiveState() async throws {
        ...
        for account in manager.accounts where account.provider == .claude {
            ...
            print("LIVE-DUMP \(account.label) [\(account.id.uuidString.prefix(8))] connected=\(state.isConnected) remaining=\(state.remainingPercent.map { String(format: "%.0f", $0) } ?? "?") fresh=\(state.isFresh()) msg=\(state.message ?? "-") :: \(windows)")
        }
