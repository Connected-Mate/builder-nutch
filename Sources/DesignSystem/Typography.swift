...
enum Typography {
    ...
    static let percent = AppTheme.font(size: Design.fontSize(capPixels: 27), weight: .semibold)

    /// "Claude Usage". Cap height 26px.
    static let cardTitle = AppTheme.font(size: Design.fontSize(capPixels: 26), weight: .semibold)

    /// "Current session", "73% Used", "Resets in 51 min". Cap height 18px.
    static let cardBody = AppTheme.font(size: Design.fontSize(capPixels: 18), weight: .regular)
}
