...
Fourteen deterministic tests cover real child-process JSON-RPC framing, native tool roundtrip, text/SSE lifecycle, authentication, Origin rejection, concurrency reservation, cancellation, model failures, unsupported formats, fixed-origin automatic fallback, credential/header filtering, nonquota errors, exact-count rejection, and actual token-usage conversion. A real installed app-server probe returned the connected ChatGPT account and six model choices. A real Astra roundtrip called the harmless echo fixture with `HELLO_RELAY`, received the client-only result `SECRET_FIXTURE_917`, and returned exactly that value. It reported nonzero actual token usage. No filesystem operation was provided or executed in that real model fixture.
